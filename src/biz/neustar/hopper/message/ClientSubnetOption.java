 ...
 * originating client, so that an authoritative server can make decisions based
 * on this address, rather than the address of the intermediate caching name
 * server.
 * 
...
public class ClientSubnetOption extends EDNSOption {
    ...
    /**
     * Construct a Client Subnet option. Note that the number of significant
     * bits in the address must not be greater than the supplied source netmask.
     * XXX something about Java's mapped addresses
     * 
     * @param sourceNetmask
     *            The length of the netmask pertaining to the query. In replies,
     *            it mirrors the same value as in the requests.
     * @param scopeNetmask
     *            The length of the netmask pertaining to the reply. In
     *            requests, it MUST be set to 0. In responses, this may or may
     *            not match the source netmask.
     * @param address
     *            The address of the client.
     */
    public ClientSubnetOption(int sourceNetmask, int scopeNetmask,
            InetAddress address) {
        super(EDNSOption.Code.CLIENT_SUBNET);
