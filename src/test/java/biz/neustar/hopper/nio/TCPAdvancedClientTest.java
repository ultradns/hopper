...
import biz.neustar.hopper.nio.example.LoggingAdvancedClientHandler;
import biz.neustar.hopper.record.ARecord;

public class TCPAdvancedClientTest {
    ...
    public void connectTCPAdvancedNegativeI() {
        DnsServer.builder().port(0).advanced(true)
    ...
    public void connectTCPAdvanced() throws InterruptedException {
        ...
        DnsClient client = DnsClient
                ...
                .advancedClientMessageHandler(
                        new LoggingAdvancedClientHandler()).build();
    ...
    public void oneClientSerialSendsOneServerTCP() throws TextParseException,
        ...
        AdvancedMessageReceivedTrap responseReceivedTrap = new AdvancedMessageReceivedTrap(
                messageCount);
        DnsClient client = DnsClient.builder().advanced(true)
                .advancedClientMessageHandler(responseReceivedTrap).build();
    ...
    public void manyClientsOneServerTCP() throws TextParseException,
        ...
        DnsServer server = DnsServer.builder()
                .serverMessageHandler(new EchoServerHandler()).port(0).build();
        SocketAddress serverAddress = new InetSocketAddress("localhost", server
                .getLocalAddress().getPort());
        ...
        for (int i = 0; i < clientCount; i++) {
            clients.add(DnsClient.builder().advanced(true)
                    .advancedClientMessageHandler(responseReceivedTrap).build());
        }
        ...
        try {
            ...
            Assert.assertTrue(responseReceivedTrap.latch.await(25,
                    TimeUnit.SECONDS));
        } finally {
            // shut down
    ...
    private void runClientsAndServers(int messageCount, int serverCount,
        ...
        for (int i = 0; i < serverCount; i++) {
            servers.add(DnsServer.builder().port(0)
                    .serverMessageHandler(new EchoServerHandler()).build());
        }
        AdvancedMessageReceivedTrap responseReceivedTrap = new AdvancedMessageReceivedTrap(
                messageCount);
        ...
        for (int i = 0; i < messageCount; i++) {
            DnsClient client = clients.get(random.nextInt(clients.size()));
            client.sendTCP(getQuery(i), new InetSocketAddress("localhost",
                    servers.get(i % serverCount).getLocalAddress().getPort()));
        }
