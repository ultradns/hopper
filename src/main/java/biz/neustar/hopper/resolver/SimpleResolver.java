...
public class SimpleResolver implements Resolver {
    ...
    @Override
    public void setEDNS(int level, int payloadSize, ExtendedFlag flags, List<EDNSOption> options) {
        if (level != 0 && level != -1)
        ...
        queryOPT = new OPTRecord(payloadSize, 0, level, flags.getValue(), options);
    }

    public void setEDNS(int level) {
        setEDNS(level, 0, ExtendedFlag.valueOf(0), null);
    }
    ...
    public Message send(Message query) throws IOException {
        ...
        do {
            ...
            if (!tcp && !ignoreTruncation
                    && response.getHeader().isFlagSet(Flag.TC)) {
                tcp = true;
                continue;
            }
            return response;
